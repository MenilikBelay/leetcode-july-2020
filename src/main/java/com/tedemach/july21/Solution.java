...
class Solution {
    ...
    public boolean exist(char[][] board, String word) {
        if (board == null || board.length == 0 || word == null || word.trim() == "") return false;
        this.board = board;
        this.word = word;
    ...
    private boolean DFS (int row, int col, int index) {
        if (index >= word.length()) return true;    // the word is over, found it that means
...
/*
R rows and C columns and L characters in the string
Running Time:  
    createBoardAndFill: O(n * m)
    DFS: O(min((n * m)!, 4^L))
    for-loop: O(n * m * O(DFS))
*/